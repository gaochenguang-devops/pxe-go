...
**子网地址池（纯子网方式）**：地址池完全由「子网管理」中的子网列表定义，不再有独立的"基础地址池"字段。点击「+ 新增子网」，每个子网独立设置名称、地址池、掩码、网关、DNS。服务器根据客户端请求匹配对应子网并下发该子网的网络参数；**无匹配子网时拒绝分配**。

**子网识别机制**（按优先级）：
1. **GIADDR**（跨网段，DHCP 中继）：三层交换机 VLANIF 开启中继后，将客户端所在 VLAN 的网关 IP 填入 GIADDR 单播转发。服务器据此确定网段。**只要存在 GIADDR，就一定经过中继**。
2. **来源 IP**（客户端已持有 IP 的情况，如续约）。
3. **服务器监听 IP / 本机接口 IP**（同网段直连）：客户端与服务器同网段时，DISCOVER 是广播、源 IP 为 `0.0.0.0`、无 GIADDR。此时服务器用**接收接口所在网卡 IP** 匹配子网——因此「监听 IP」建议填写服务器在该网段的实际接口 IP（而非 `0.0.0.0`），或服务器网卡 IP 已落在对应子网内。

配置结构与传统 ISC DHCP 对应关系（参考）：
```
subnet 10.122.240.128 netmask 255.255.255.192 {   →  子网
    option routers 10.122.240.190;                →  gateway
    range 10.122.240.169 10.122.240.187;          →  ip_pool_start / ip_pool_end
}
```

适用于一台上层服务器服务多个 VLAN/网段的场景（子网间通常通过 DHCP 中继转发请求）。修改保存后**自动热加载**，无需重启进程。

> **升级迁移**：从旧版本（单网段字段）升级时，服务启动会自动把旧的 `ip_pool_start/ip_pool_end/subnet_mask/gateway/dns_servers` 配置迁移为一条「默认子网」记录（仅当子网列表为空时执行），并清理旧配置键，**无需手动干预**。全新安装默认生成一条 `192.168.10.100-200/24` 的默认子网。
